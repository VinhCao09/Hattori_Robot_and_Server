...
HTTPClient http;
//host to send data
const char* host= "http://192.168.1.3/ESP32_Control_Robot/Getstatus.php";
...
//----BIEN CHO SERVO WHEN NORMAL -------------
String message = "", message1= "";// Khai báo biến message để lưu dữ liệu
String oldprompt = "";
#define LED 4
int lenh = -1; 
#include "xu_ly_du_lieu_voice.h"
#include "api_gemini.h"
...
String NHAN_DATA_TU_BLUETOOTH()
{
  ...
  if (message.length() > 0) {
   ...
   lenh = handleMessage(message1); DIEUKHIENTHIETBI(lenh); 
   return message1;
}
void GEMINI(String prompt)
{
  if(oldprompt != prompt){
    tft.fillScreen(BLACK);  
    String me = "Bạn đã hỏi: " + prompt;
    // Gửi yêu cầu và in kết quả
    //--------IN RA CAU HOI CUA BAN:-------------
        int y = 0;
        int start = 0;
        int end = 0;
        for (;;) { // Sử dụng vòng lặp vô hạn
          end = me.indexOf(" ", start + 40); // Tìm vị trí của dấu cách sau 10 từ
          if (end == -1) {
            end = me.length(); // Nếu không tìm thấy dấu cách, kết thúc chuỗi
          }
          String line = me.substring(start, end);
          myfont.print(0, y, line, YELLOW, BLACK);
          y += 15; // Khoảng cách giữa các hàng là 10 pixel
          start = end + 1; // Bắt đầu từ vị trí ký tự tiếp theo
          if (start >= me.length()) {
            break; // Nếu đã xử lý hết chuỗi, thoát khỏi vòng lặp
          }
        }
  //--------IN RA CAU HOI CUA BAN:-------------
    String response = askHattori(prompt);
    oldprompt = prompt;
    
    Serial.println("Response from Hattori: " + response);
    // myfont.print(0,y,response,RED,BLACK);
  //--------IN RA TRA LOI TU GEMINI:-------------
  String AI = "Hattori: " + response;
        y = 50;
        start = 0;
        end = 0;
        for (;;) { // Sử dụng vòng lặp vô hạn
          end = AI.indexOf(" ", start + 40); // Tìm vị trí của dấu cách sau 10 từ
          if (end == -1) {
            end = AI.length(); // Nếu không tìm thấy dấu cách, kết thúc chuỗi
          }
          String line = AI.substring(start, end);
          myfont.print(0, y, line, RED, BLACK);
          y += 15; // Khoảng cách giữa các hàng là 10 pixel
          start = end + 1; // Bắt đầu từ vị trí ký tự tiếp theo
          if (start >= AI.length()) {
            break; // Nếu đã xử lý hết chuỗi, thoát khỏi vòng lặp
